...
WiFi 自动切换的内部绕过模式。普通业务流量使用专用的 `wifi-direct` 直连出站，
该模式的路由规则位于嗅探和 `hijack-dns` 之前，因此连接与 DNS 请求都保持
原始目标并交给当前 WiFi 网关；不会进入手机侧 DNS 模块或直连 DoH，避免绕过
家庭透明代理策略。
该模式由 SSID 策略自动选择，不作为持久化的 `OUTBOUND_MODE`。
