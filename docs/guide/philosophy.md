...
NetProxy 8.0 想解决的，不只是“把代理跑起来”，而是把 Android 上本来复杂、割裂、门槛高的代理体验，整理成一套更顺手、更可靠的日常工具。
...
3. **Clash API + zashboard**
   面向运行时观察、切换和连接查看。
4. **Service API + sing-box Dashboard**
   面向 sing-box 原生状态、节点组、连接和测速。
...
- Catalog 保存持久节点，Service API 提供运行时状态，避免两份状态互相覆盖
...
- eBPF 只是 sing-box 的透明代理入站，不把内核能力包装成另一套代理核心
