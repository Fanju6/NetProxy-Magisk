package com.fanjv.netproxy.feature.nodes.presentation.edit.components
...
internal fun TlsConfigSection(
    ...
    Column(verticalArrangement = Arrangement.Top) {
        ...
        if (enabled) {
            EditorField(serverName, stringResource(R.string.server_name), onServerNameChange, onImeDone)
            Card(modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp).padding(bottom = 12.dp)) {
                SwitchPreference(
            ...
            if (realityEnabled) {
                EditorField(realityPublicKey, realityPublicKeyLabel, onRealityPublicKeyChange, onImeDone)
            ...
            if (echEnabled) {
                ...
                EditorField(echQueryServerName, echDnsServerNameLabel, onEchQueryServerNameChange, onImeDone)
            }
        }
    }
}
