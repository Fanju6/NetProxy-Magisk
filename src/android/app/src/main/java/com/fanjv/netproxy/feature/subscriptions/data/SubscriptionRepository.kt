package com.fanjv.netproxy.feature.subscriptions.data
...
internal class SubscriptionRepository(
    ...
    suspend fun cancelUpdate(id: String) {
        client.execute("sub", "cancel", id)
    }

    suspend fun history(id: String): List<SubscriptionHistoryEntry> =
        client.json.decodeFromJsonElement(client.execute("sub", "history", id).data)
    ...
    private suspend fun <T> withHeadersFile(
        ...
        val content = client.json.encodeToString(
            JsonObject(headers.mapValues { JsonPrimitive(it.value) })
        )
        return commandFiles.withTextFile("netproxy-headers-", ".json", content, block)
    }
}
