...
#######################################
# 文件: service.sh
# 功能: 无参数时执行模块开机流程；带参数时管理 sing-box 生命周期。
...
readonly WORKER_PID_FILE="/dev/netproxy/subworker.pid"
...
不传参数时由 Magisk/KernelSU/APatch 在 service 阶段执行开机流程。
...
start_subscription_worker() {
  if "$NETPROXY_NATIVE_BIN" subworker start \
    ...
    --reload-script "$MODDIR/service.sh" \
    --sing-box "$SING_BOX_BIN" \
    --service-address "127.0.0.1:9090" \
    --service-secret "singbox" > /dev/null 2>&1; then
    boot_log "DEBUG" "订阅自动更新 Worker 已就绪"
  else
    boot_log "WARN" "订阅自动更新 Worker 启动失败，可稍后手动重试"
  fi
}
...
log_boot_environment() {
  if [ "${KSU:-}" = "true" ]; then
    boot_log "DEBUG" "环境: KernelSU ${KSU_VER:-未知}"
  elif [ "${APATCH:-}" = "true" ] || [ "${KERNELPATCH:-}" = "true" ]; then
    boot_log "DEBUG" "环境: APatch / KernelPatch"
  elif [ -n "${MAGISK_VER:-}" ]; then
    boot_log "DEBUG" "环境: Magisk ${MAGISK_VER}"
  fi

  if [ -f "$MODDIR/module.prop" ]; then
    local version version_code
    version="$(sed -n '1s/^version=//p' "$MODDIR/module.prop")"
    version_code="$(sed -n '1s/^versionCode=//p' "$MODDIR/module.prop")"
    boot_log "模块版本: ${version:-未知} (${version_code:-未知})"
  fi
}
...
boot_main() {
  set -e
  mkdir -p "$MODDIR/logs"
  boot_log "NetProxy 开机服务启动 (service 阶段)"
  log_boot_environment
  load_boot_config
  wait_for_boot

  if [ "$AUTO_START" = "1" ]; then
    if ! (service_main start); then
      boot_log "WARN" "代理服务开机启动失败，可在导入节点或修正配置后手动启动"
    fi
  else
    boot_log "开机自启动已禁用，跳过启动"
  fi
...
case "${1:-}" in
  "") boot_main ;;
  start | stop | restart | reload | status | check | help | -h | --help) (service_main "$@") ;;
  *)
    show_usage
    exit 2
    ;;
